...
const Collapsible = ({id, name, depth, onRemoveDataRow, props}) => {
    ...
    const removeDataRow = (parrents) => {
        onRemoveDataRow(parrents);
    };
    return (<div className="menuListItem" >
                ...
                <Collapse isOpened={isOpen}>
                    {props.states ? <SidebarItem data={props.states} depth={depth+20} onRemoveDataRow={onRemoveDataRow} parrents ={parrents} /> : null }
                    {props.cities ? <SidebarItem data={props.cities} depth={depth+20} onRemoveDataRow={onRemoveDataRow} parrents ={parrents} /> : null }
                </Collapse> 
                :null}
            </div>);
}

const SidebarItem = ({data, depth=0, onRemoveDataRow=null, parrents=[] }:{data:Array<itemType>, depth:number, onRemoveDataRow:any, parrents: Array<any>}) => 
{
    return (
        <div className="menuList">
            {
                data.map( ({name, id, ...props}) => {
                    props = { ...props , ...{parrents}};
